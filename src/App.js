    ...
    <div style={{ minHeight: "100vh", background: "#E5E0D5" }}>
      <div style={{
        ...
        <div style={{
          ...
          {!isLast && (import React, { useState } from "react";
import "./App.css";
      ...
      {options.map(opt => {
        ...
        return (
          <button key={opt} onClick={() => onChange(opt)} style={{
            background: on ? C.green : C.bg, color: on ? "#fff" : C.text,
  ...
  return (
    <div style={{ background: C.card, borderRadius: 14, border: `1.5px solid ${C.border}`, marginBottom: 10, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: C.greenLight, borderBottom: `1px solid ${C.border}` }}>
        ...
        <button onClick={onRemove} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: C.muted, padding: "0 4px" }}>✕</button>
      </div>
...
function InfantExtra({ person, onChange }) {
  const isInfant = INFANT_AGES.includes(person.age);
    ...
    <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {["Breastfed", "Formula fed", "Mixed feeding"].map(opt => {
          ...
          return (
            <button key={opt} onClick={() => onChange({ ...person, feeding: opt })} style={{
              background: on ? C.green : C.bg, color: on ? "#fff" : C.text,
              border: `1.5px solid ${on ? C.green : C.border}`,
              borderRadius: 99, padding: "7px 14px", fontSize: 13,
              fontWeight: on ? 700 : 400, cursor: "pointer",
      ...
      <InfoBox color={C.warn} borderColor={C.warnBorder} textColor={C.warnText}>
        ⚠️ <strong>Formula will be included as an emergency backup</strong> regardless of feeding method — in case the feeding parent is unable to feed. Guidelines recommend this for all infants.
      </InfoBox>
      ...
      {[["Yes", true], ["No", false]].map(([label, val]) => {
        ...
        return (
          <button key={label} onClick={() => onChange(val)} style={{
            flex: 1, background: on ? C.green : C.bg, color: on ? "#fff" : C.text,
            border: `1.5px solid ${on ? C.green : C.border}`, borderRadius: 12,
  ...
  return (
    <>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
              {["Yes — responsible", "No — just a buffer"].map(opt => {
                ...
                return (
                  <button key={opt} onClick={() => updateGuests("responsible", opt)} style={{
                    flex: 1, background: on ? C.greenLight : C.bg, color: on ? C.green : C.text,
                    border: `1.5px solid ${on ? C.green : C.border}`, borderRadius: 12,
                    padding: "10px 8px", fontSize: 13, fontWeight: on ? 700 : 400, cursor: "pointer",
                  }}>{opt}</button>
                );
              })}
            </div>
      ...
      <Card>
        ...
        <YesNo value={d.specialistNeeds ?? null} onChange={v => set(p => ({ ...p, specialistNeeds: v }))} />
      </Card>
    </>
  );
}

// --- SCREEN 1b: Specialist Needs holding page ---
function S1b({ d, set }) {
  return (
    <>
      ...
      <Card style={{ textAlign: "center", padding: "32px 20px" }}>
        ...
        <div style={{ background: C.greenLight, borderRadius: 12, padding: "14px 16px", fontSize: 13, color: C.green, lineHeight: 1.6, textAlign: "left" }}>
          ✅ We've noted that your household has specialist needs. This will be factored into your plan as we develop this feature.
        </div>
      </Card>
      <InfoBox>
        📋 <strong>Guidelines suggest</strong> discussing emergency planning with your GP or specialist if any household member has significant medical dependencies or equipment needs.
      </InfoBox>
    </>
  );
}
  ...
  return (
    <>
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: C.bg, borderRadius: 12, padding: "14px", border: `1.5px solid ${C.border}`, marginBottom: 10 }}>
            <span>🔍</span><span style={{ fontSize: 15, color: C.muted }}>Search your address...</span>
          </div>
...
function S7({ d, set }) {
  ...
  return (
    <>
      ...
      <InfoBox>
        ⚠️ <strong>Your situation:</strong> Canterbury rural + earthquake + supply cut-off. Roads could be blocked for weeks. <strong>Guidelines suggest 8 weeks minimum</strong> for your region.
      </InfoBox>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
        {durations.map(dur => {
          const on = sel === dur.id;
...
export default function App() {
  ...
  const nextScreen = getNext(current, data);
  const prevScreen = getPrev(current, data);
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: C.bg, padding: "14px 20px 10px", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: C.green }}>{current === "1b" ? "Specialist Needs" : PROGRESS_SCREENS[progressIndex]?.title}</span>
          </div>
          <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
          ...
          <div style={{ display: "flex" }}>
            {PROGRESS_SCREENS.map((s, i) => (
              <button key={s.id} onClick={() => setCurrent(s.id)} style={{ flex: 1, border: "none", background: "none", cursor: "pointer", fontSize: 18, opacity: i === progressIndex ? 1 : 0.3, padding: "2px 0" }}>{s.icon}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: "14px 20px 24px", borderTop: `1px solid ${C.border}`, background: C.bg, display: "flex", gap: 10, flexShrink: 0 }}>
          ...
          {nextScreen && (
            <button onClick={() => setCurrent(nextScreen)} style={{ flex: 2, background: C.green, border: "none", borderRadius: 14, padding: "16px", fontSize: 16, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "Georgia, serif" }}>Continue →</button>
          )}
        </div>
